  ...
  // The perceptual (Lab ΔE) image comparison shared by the `NSImage`/`UIImage` diffing strategies,
  // ported from the legacy witness implementation. The algorithm is kept byte-for-byte equivalent —
  // including its failure messages — so that assertions relying on `perceptualPrecision < 1` behave
  // identically to the legacy library. The legacy `@available` guards are dropped: the package's
  // platform floor (macOS 13 / iOS 16 / tvOS 16) exceeds all of them.
  ...
  func perceptuallyCompare(
    ...
    // Setting the working color space and output color space to NSNull disables color management.
    // This is appropriate when the output of the operations is computational instead of an image
    ...
    // Metal is supported by all hardware that can run the package's platform floor. However, some
    // virtualized hardware do not have GPUs and therefore do not support Metal. In this case,
    // macOS falls back to a CPU-based OpenGL ES renderer that silently fails when a Metal command
    // is issued. We need to check for Metal device support and fall back to CPU-based vImage
    // buffer iteration.
    if ThresholdImageProcessorKernel.isSupported {
      ...
      // rowBytes must be a multiple of 8, so vImage_Buffer pads the end of each row with bytes to
      // meet the multiple of 8 requirement. We must do 2D iteration of the vImage_Buffer in order
      // to avoid loading the padding garbage bytes at the end of each row.
      //
    ...
    // The actual perceptual precision is the perceptual precision of the pixel with the highest
    // DeltaE. DeltaE is in a 0-100 scale, so we need to divide by 100 to transform it to a
    // percentage.
  ...
  extension CIImage {
    ...
    {
      // Some hardware configurations (virtualized CPU renderers) do not support 32-bit float
      // output formats, so use a compatible 16-bit float format and convert the output value to
      // 32-bit floats.
      guard
